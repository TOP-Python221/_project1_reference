...
def print_board(board: Matrix, *boards, right: bool = False) -> None:
    ...
    # формирует горизонтальную линию разделитель на основе данных о полной ширине всех полей, добавляя между линиями пробелы pad
    horiz_lines = (' '*pad).join('—'*wd for wd in width_boards)
    # заканчивает формирование горизонтальных разделителей добавляя отступ margin и символы конца строк
